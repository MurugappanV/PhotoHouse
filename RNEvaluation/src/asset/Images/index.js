/**
 * Source for images
 * Author : Murugappan V
 * Date   : 9 Sep 2018
 * @flow
 */

export const Images = {
	wifiImg: require("./wifi.png"),
	mapAtmImg: require("./atm_icn.png"),
	mapBankImg: require("./bank_icn.png"),
	mapUserImg: require("./place.png"),
	atmImg: require("./atm1_icn.png"),
	directionImg: require("./direction_icn.png"),
	filterImg: require("./filter_icn.png"),
	searchImg: require("./Shape.png"),
	upImg: require("./arrow1_icn.png"),
	leftImg: require("./arrow_left_icn.png"),
	rightImg: require("./arrow_right_icn.png"),
	backImg: require("./back_arrow.png"),
};
